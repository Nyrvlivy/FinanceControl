...
export class UpdateTransactionDialogComponent {
  ...
  onSaveClick(): void {
    ...
    this.transactionsService.updateTransaction(updatedTransaction).subscribe({
      next: () => {
        this.snackBar.open(
          `Transaction ${updatedTransaction.id} updated successfully`,
          'Close',
          { duration: 3000 },
        );
  ...
  displayErrorMessages(): void {
    if (!this.transaction.id) {
      ...
      this.snackBar.open('Category cannot be null', 'Close', {
        duration: 3000,
      });
    }
  }
